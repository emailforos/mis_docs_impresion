 ...
 * Indica el neto, iva y total para un determinado IVA y un albarán.
 */
class linea_iva_pedido_cliente extends fs_model
{
   public $totallinea;
   public $totalrecargo;
   public $recargo;
   ...
   {
      parent::__construct('lineasivapedcli', 'plugins/facturacion_base/');
      if($l)
      {
   ...
   public function save()
   {
      if( $this->exists() )
      {
         $sql = "UPDATE ".$this->table_name." SET idpedido = ".$this->var2str($this->idalbaran).",
   ...
   public function all_from_pedido($id)
   {
      ...
      {
         foreach($lineas as $l)
            $linealist[] = new linea_iva_pedido_cliente($l);
      }
