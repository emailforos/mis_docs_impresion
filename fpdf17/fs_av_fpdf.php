...
{	
	...
	$this->addCliente($this->fdf_codcliente);
        ...
        $cliente .= "Tlf: " . $this->fdf_telefono1;
        if($this->fdf_telefono2) { $cliente .= " - " . $this->fdf_telefono2 . "\n"; } else { $cliente .= "\n"; }
        if($this->fdf_fax) { $cliente .= "Fax: " . $this->fdf_fax . "\n"; }
        if($this->fdf_email) { $cliente .= "Email: " . $this->fdf_email . "\n"; }		
        $this->addClientAdresse(utf8_decode($cliente));		

        // Forma de Pago del Pedido
        $this->addPago(utf8_decode($this->fdf_epago),utf8_decode($this->fdf_cuentapago) );
        ...
        for($i=0;$i<count($this->datoscab);$i++)
        {	
            $this->Rect($aquiX, $aquiY, $this->widths[$i], 125, 1, 'D');
            $aquiX += $this->widths[$i];
        }
    }
    ...
    function Footer()
    {
        ...
        if ($this->piepagina == true)
        {
            ...
            {
                $this->addObservaciones(substr($this->fdf_observaciones, 0, 150/*116*/));
            }
    ...
    function fact_dev( $libelle, $num )
    {
        ...
        $this->SetXY($w,14);
        $this->Cell(0,6,'Albar'.chr(225).'n',0,0,'R');
        $this->SetFont("Arial", "B", 12 );
        $this->SetXY($w,19);
    ...
    function addClientAdresse( $adresse )
    {
        ...
        $this->SetTextColor(0);
        $length = $this->GetStringWidth( $nom );
        $this->MultiCell(66,5,'Destinatario: ');
        $this->SetXY( $r1, $y1);
	  $this->Rect ($r1-2,$y+42,89,40,'D');
    ...
    function addPago( $mode, $account )
    {
        ...
        $y2  = $y1+10;
        $mid = $y1 + (($y2-$y1) / 2);
	  $texte  = 'Forma de pago: ' . $mode . $account;    
        $this->SetXY(10,250/*$h-41,5*/);
        $this->SetFont( "Arial","B",9);
    ...
    function addObservaciones($observa)
    {
        ...
        $length = $this->GetStringWidth( "Observaciones: ". $observa );
        $this->SetXY( 10, 263/*$this->h - 37.5*/ );
        $this->Cell($length,0, "Observaciones:");
        $this->SetXY( 35, 263/*$this->h - 37.5*/ );
        $this->SetFillColor(230,230,230);
        $this->SetFont( "Arial", "I", 8);
        $this->MultiCell(0,4, $observa,'0','L','true');
    }
